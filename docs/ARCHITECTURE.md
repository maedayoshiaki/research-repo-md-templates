...
設計判断は複数案から人間が選ぶ。ただし、判断の所有者と AI への委任範囲は `AGENTS.md` の Repository Human Involvement Profile に従う。

## Table of Contents
1. [Design Principles](#design-principles)
2. [Design Decision Ownership by Profile](#design-decision-ownership-by-profile)
3. [Architectural Style Options（規模別 + 選択基準）](#architectural-style-options規模別--選択基準)
4. [Module Map](#module-map)
5. [Dependency Rules](#dependency-rules)
6. [Design Pattern Catalog](#design-pattern-catalog)
7. [Research-Specific Structures](#research-specific-structures)
8. [Anti-Patterns & Anti-Over-Engineering](#anti-patterns--anti-over-engineering)
...
## Design Decision Ownership by Profile

設計判断の所有者は Repository Human Involvement Profile に従う。

| Profile | 設計判断 | パターン採用 | 実装詳細 |
|---|---|---|---|
| `L0 Speed First` | AI が提案し、人間が確認 | AI が選んでよい | AI 主導 |
| `L1 Review Gate` | AI が提案し、人間がレビュー | AI が採用理由を書く | AI 主導 |
| `L2 Selective Human Slots` | AI が 2〜5 案を出し、人間が選ぶ | 人間が採否を選ぶ | 分担 |
| `L3 Learning First` | 人間が選ぶ。AI は比較表を出す | 人間が採否を説明する | 人間中心 |
| `L4 Human Driver` | 人間が主導 | 人間が実装・説明する | 人間主導 |

Profile に関係なく、次は人間レビューを必要とする。
...
コーディングエージェントが subagent を使える場合は、設計案の追加調査、既存コード探索、テスト失敗の切り分けに使ってよい。
ただし、subagent は設計判断の所有者ではない。採用判断、Profile からの逸脱、人間レビューが必要な判断は、main agent が統合して人間に提示する。
...
GoF の 3 分類で整理（Factory はその一例にすぎない）。**まず素朴に書き、重複・分岐の散在・テスト困難が見えてから**入れる。採用時は理由を 1〜3 文添える。
採用判断の主担当は Repository Human Involvement Profile に従う。
...
- 迷ったら AGENTS.md に従い、**素朴案 vs 抽象案**を並べて人間に選ばせる。
