...
`L3` では、人間が文脈のない小さな穴埋めだけを担当しないようにする。抽象、代表的な具象実装、最小の呼び出し例、契約テストを一続きの `Design Vertical Slice` として扱う。
...
| Slice | 人間が書くもの | AI が保持・補助する文脈 | 学習目的 | 完了条件 |
|---|---|---|---|---|
| Interface Slice | 抽象インターフェース、責務、最小契約 | 既存呼び出し側、命名、候補メソッド | DIP / ISP / 契約設計 | 具象クラスを差し替えられる |
| Concrete Slice | 代表的な具象クラス 1 つ | 類似実装、境界条件、レビュー | 多態性、Strategy / Adapter など | インターフェース経由で動く |
| Usage Slice | 最小の呼び出し例 | 統合先、設定、Factory / Registry 案 | 抽象と利用側の分離 | 呼び出し側が具象に依存しない |
| Contract Test Slice | 契約テスト、失敗例 | テスト観点、fixture、boilerplate | 置換可能性の検証 | 新しい具象実装にも同じテストを適用できる |

AI が実装してよい部分：

- {{例: CLI 引数、ログ出力、テスト雛形、設定ファイル、単純なリファクタ、類似する 2 個目以降の具象クラス}}

AI が完成実装を先に出すべきでない部分（主に `L2` 以上）：

- {{例: 研究アルゴリズムの核、モデル forward、評価指標の本体、設計パターンの採否判断、L3 の最初の Interface / Concrete / Usage / Contract Test slice}}
...
- どこを変更すれば拡張できるか：{{...}}
- どのテストで正しさを確認するか：{{...}}
- `L3` の場合、抽象・代表具象・呼び出し側・契約テストがどうつながるか：{{...}}
